  ...
  public void insert(Object o) throws SQLException, Exception {
    ...
    for (int i = 0; i < m.length; i++) {
      ...
      if (methodName.indexOf("get") == 0 && !methodName.equals("getClass")) {
        ...
        Boolean b = (Boolean) c.getMethod(isHasMethod, null).invoke(o, null);
        if (b.booleanValue()) {
    ...
    for (int i = 0; i < m.length; i++) {
      ...
      if (methodName.indexOf("get") == 0 && !methodName.equals("getClass")) {
        ...
        if (b.booleanValue()) {
          ...
          Boolean isWhereFlag = (Boolean) c.getMethod(isWhere, null).invoke(o, null);
...
//      }
  ...
  /**The following method is used by DBParser
     /**
    * execute databse query with no resultset return
    * **/
   ...
   */
  public Hashtable getDataWithNo(String table, String swhere) throws Exception {
    Connection dbConnection = this.getConnection();
   ...
   * @return  record count is > 1
   */
  public Vector getData(String table, String swhere, int begin, int end) throws
      Exception {
    Vector over = new Vector();
    Connection dbConnection = this.getConnection();
    Hashtable bdData = new Hashtable();
  ...
  public void insertUserData(String id, String name, String pass1, String role) throws
      SQLException {
  ...
  public void updateUserData(String id, String name, String pass1, String role) throws
      SQLException {
    this.connection = this.getConnection();
    PreparedStatement ps = this.connection.prepareStatement(
        "update user set password = ?,userName=?,role = ? where userID = ?");
  ...
  public void insertProductTypeData(String id, String name, int type) throws
      SQLException {
    PreparedStatement ps = this.connection.prepareStatement(
  ...
  public void updateProductTypeData(String id, String name) throws SQLException {
    PreparedStatement ps = this.connection.prepareStatement(
        "update productType set name = ? where id = ?");
    ps.setString(1, name);
    ps.setString(2, id);
    ps.executeUpdate();
    ps.close();
  ...
  public void updateProductData(Vector data, String id) throws SQLException {
    ...
    sql =
        "update product set name = ?,brand = ?,spec = ?,price = ?,inputprice = ?,"
  ...
  public Vector selectData(String sql) throws SQLException {
    ...
    while (rs.next()) {
      ...
      for (int i = 0; i < rs.getMetaData().getColumnCount(); i++) {
        ...
        if (o_temp== null || o_temp.toString().equals("-1") || o_temp.toString().equals("&^%")) {
          temp.addElement("");
        }
        else {
  ...
  public String getOrderID() throws SQLException {
    ...
    ResultSet rs = st.executeQuery(
        "select max(orderID) from order1 where orderID like '" + s_date +
        "%' and isTemplate = 2 ");
    String id = "";
    if (rs.next()) {
      id = rs.getString(1);
  ...
  public String getTemplateID() throws SQLException {
    Statement st = this.connection.createStatement();
    ResultSet rs = st.executeQuery(
        "select max(orderID) from order1 where  isTemplate = 1 ");
    String id = "0";
    if (rs.next()) {
      id = rs.getString(1);
    }
  ...
  public Vector getOrderData(String s_where) throws SQLException {
    ...
    while (rs.next()) {
      ...
      for (int i = 0; i < colcount; i++) {
        Object o_temp = rs.getObject(i + 1);
        if (o_temp.toString().equals("-1") || o_temp.toString().equals("&^%")) {
          temp.addElement("");
        }
        else {
          temp.addElement(o_temp);
  ...
  public Vector getTemplateData() throws SQLException {
    Vector data = new Vector();
    String sql = "select orderid, customerName, money from  order1 where isTemplate = 1 order by orderid desc ";
    System.out.println("sql = " + sql);
    Statement st = this.connection.createStatement();
    ResultSet rs = st.executeQuery(sql);
    while (rs.next()) {
  ...
  public void insertOrUpdateOrderData(Vector maindata, Vector childData,
                                      Vector productID, String id,
                                      boolean isOrder) throws SQLException {
    this.connection.setAutoCommit(false);
    Statement st = this.connection.createStatement();
    ...
    try {
      ...
      for (int i = 0; i < childData.size(); i++) {
        ...
        int quantity_present = Integer.parseInt(temp.elementAt(5).toString());
        if (rs.next()) {
  ...
  public void deleteOrderData(String id, boolean isOrder) throws SQLException {
    ...
    String sql_quantity =
        "select productid,count from orderchild where orderid = '" + id + "'";
    ResultSet rs = st.executeQuery(sql_quantity);
    try {
      //while(rs.next()){
...
//  rs.close();
  ...
  public void updateProductQuantity(String productID, int quantity,
    ...
    if (!isInput) {
      if (quantity_or < quantity) {
        st.close();
        throw new SQLException("APP产品ID: " + productID + " 库存小于出货量,无法出货");
      }
    }
    if (isInput) {
  ...
  private static DBAgent_Single dbAgent = new DBAgent_Single();
  public static  DBAgent_Single getInstance(){
    return dbAgent;
  }
