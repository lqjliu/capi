...
import com.common.util.ColumnProfile;
...
/**
 * return the column type of comments in hashtable, key: columnName,value : label
 * @param tableName
 * @return
 * @throws Exception
 */
...
/**
 * return the column label of comments in hashtable, key: columnName,value : label
 * @param tableName
 * @return
 * @throws Exception
 */
  public Hashtable getColumnLabel(String tableName) throws Exception{
    String tablename1 = tableName.toUpperCase();
    String sql = "select column_name,comments from user_col_comments where table_name='" + tablename1 + "'";
    DBAgent_Parse dbAgent = new DBAgent_Parse();
    Vector data = dbAgent.selectMultiRecord(sql);

    sql = "select count(*) from user_tab_columns where table_name='" + tablename1 + "'";
    int count = Integer.parseInt(dbAgent.selectSingleValue(sql));

    if(data.size() != count)
      throw new Exception("Table : " + tableName + " column comments count not equals column count");
    Hashtable ht = new Hashtable();
    for(int i = 0; i< data.size(); i++){
      Vector temp = (Vector)data.elementAt(i);
      ht.put(temp.elementAt(0).toString().toLowerCase(),temp.elementAt(1).toString());
    }
    return ht;
  }
...
/**
 * return the column data length of comments in hashtable, key: columnName,value : label
 * @param tableName
 * @return
 * @throws Exception
 */
  public Hashtable getColumnDataLength(String tableName) throws Exception{
    String tablename1 = tableName.toUpperCase();
    String  sql = "select * from user_tab_columns where table_name='" + tablename1 + "'";
    ...
    for(int i = 0; i< data.size(); i++){
      Vector temp = (Vector)data.elementAt(i);
      ht.put(temp.elementAt(1).toString().toLowerCase(),temp.elementAt(5).toString());
    }
    return ht;
  }
 ...
 */
  public String getTableLabel(String tableName) throws Exception{
    String tablename1 = tableName.toUpperCase();
    String sql = "select comments from user_tab_comments where table_name='" + tablename1 + "'";
    DBAgent_Parse dbAgent = new DBAgent_Parse();
   ...
   * @param args, vector No.1 child column, No.2 father table, No.3 father column
   */
  public Vector getReferenceColumn(String tableName) throws Exception{
    ...
    String sql = " select a.c_child,b.t_father,b.c_father from ("
               ...
               + " and constraint_type='R' and user_cons_columns.table_name='" + tablename1 + "' ) a,"
               + " ("
